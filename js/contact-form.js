(() => {
  ...
  const validate=()=>{let first=null;form.querySelectorAll('[required]').forEach(field=>{const valid=field.checkValidity();field.setAttribute('aria-invalid',String(!valid));if(!valid&&!first)first=field;});const message=form.elements.namedItem('message');if(message.value.trim().length<10){message.setCustomValidity('Message must be at least 10 characters.');first||=message;}else message.setCustomValidity('');if(!first)return true;show('Please check all required fields and formats. The message must be at least 10 characters.','error');first.focus();return false;};
  ...
  form.addEventListener('submit',async event=>{
    ...
    if(!validate())return;const key='portfolio_contact_last_submit';if(Date.now()-Number(sessionStorage.getItem(key)||0)<60000){show('Please wait about one minute before sending another message.','error');return;}submit.disabled=true;submit.setAttribute('aria-busy','true');submit.dataset.originalLabel||=submit.textContent;submit.textContent='Sending…';show('Sending your message…','pending');try{await issueToken();const response=await fetch(form.action,{method:'POST',body:new FormData(form),headers:{Accept:'application/json'},mode:'cors',credentials:'omit'});const result=await response.json().catch(()=>({ok:false,code:'invalid_response'}));if(!response.ok||!result.ok)throw new Error(result.code||'send_failed');const inquiryType=String(form.elements.namedItem('inquiryType').value||'general');window.dataLayer=window.dataLayer||[];window.dataLayer.push({event:'generate_lead',source_site:'koshijpn.github.io',form_name:'github_portfolio_contact',inquiry_type:inquiryType,page_location:location.href.split('?')[0]});sessionStorage.setItem(key,String(Date.now()));form.reset();formStarted=false;setMetadata();show('Message sent. I usually reply within 2–3 business days.','success');}catch(error){show(error.message==='rate_limited'?'Please wait before sending another message.':error.message==='server_not_configured'?'The contact service has not been configured yet.':error.message==='csrf_unavailable'?'The security token could not be created. Please wait and try again.':'Your message could not be sent. Please wait and try again.','error');}finally{submit.disabled=false;submit.removeAttribute('aria-busy');submit.textContent=submit.dataset.originalLabel;}});
})();
