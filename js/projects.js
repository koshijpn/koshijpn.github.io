...
const skillGroups = [
  { title: "Front-end", items: ["HTML", "CSS", "JavaScript", "React", "Tailwind CSS", "Responsive Design"] },
  { title: "CMS / Web", items: ["WordPress", "Divi", "WooCommerce", "Multilingual Websites"] },
  { title: "Development", items: ["Git", "GitHub", "REST APIs", "API Integration", "Deployment"] },
  { title: "AI-assisted Development", items: ["AI coding tools", "Rapid prototyping", "Workflow automation", "Human-reviewed implementation"] },
  { title: "Growth", items: ["SEO", "GA4", "Google Tag Manager", "Microsoft Clarity", "Search Console"] }
];
...
function createLink(label, url) { const a=document.createElement("a"); a.href=url; a.target="_blank"; a.rel="noopener noreferrer"; a.textContent=`${label} ↗`; if(label.toLowerCase()==="demo")a.dataset.track="portfolio_demo_click";else if(url.includes("github.com/"))a.dataset.track="github_click";return a; }
...
function renderAll(){renderSkills();renderProjects();document.querySelectorAll("a[href]").forEach(link=>{if(!link.getAttribute("href")?.startsWith("#"))link.href=window.KoshiLocale?.localizeUrl(link.href,language())||link.href;});}
